...
int main() {
  ...
  int j, max, flag, maxpos;
  for(i = 0; i < no_process; ++i) {
    max = 0, flag = 0;
    for(j = 0; j < n; ++j) {
      //printf("\nmax = %d max size = %d j size = %d",max,p[max].size, p[j].size );
      if(!p[j].allocated && pro[i].mem <= p[j].size && p[j].size >= max){
        max = p[j].size;
        maxpos = j;
        flag = 1;
      }
    ...
    if(flag) {
      p[maxpos].allocated = 1;
      p[maxpos].fragment = p[maxpos].size - pro[i].mem;
      pro[i].part = p[maxpos].id; //allocated partition
      ext -= p[maxpos].size;
    }
    pro[i].ext = ext;
  }
